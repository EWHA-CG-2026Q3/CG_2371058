...
public class S06_OpenGL_Finish : MonoBehaviour
{
    ...
    void OnEnable() { UnityEngine.Rendering.RenderPipelineManager.endCameraRendering += DrawTriangle; }
    void OnDisable() { UnityEngine.Rendering.RenderPipelineManager.endCameraRendering -= DrawTriangle; }
    void DrawTriangle(UnityEngine.Rendering.ScriptableRenderContext context, Camera camera)
    {
        if (glMaterial == null || camera != Camera.main) return;
        GL.PushMatrix();
        GL.LoadProjectionMatrix(camera.projectionMatrix);
        GL.modelview = camera.worldToCameraMatrix;
        // URP의 카메라 렌더링 완료 이벤트에서 GL로 삼각형을 그린다.
